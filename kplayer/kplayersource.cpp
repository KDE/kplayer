  ...
  : KPlayerSource (parent)
{
  ...
  m_directory.setFilter (QDir::AllEntries);
...
bool checkMimeType (const QString& path)
{
  ...
  const QString& name (mimetype -> name().toLower());
#ifdef DEBUG_KPLAYER_SOURCE
...
{
  return info.fileName() != "." && info.fileName() != ".." && info.exists() && info.isReadable()
...
void KPlayerDirectorySource::enumStart (bool groups)
{
  ...
  kdDebugTime() << " ID     " << parent() -> id() << "\n";
#endif
  m_list = m_directory.entryInfoList();
  m_iterator = m_list.constBegin();
#ifdef DEBUG_KPLAYER_SOURCE
  kdDebugTime() << " Count  " << m_list.count() << "\n";
#endif
}
