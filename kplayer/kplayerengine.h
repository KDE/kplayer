...
public: 
  ...
  KPlayerFileDialog (const QString& dir, const QString& filter, QWidget* parent);

  /** Returns the last used directory. */
  QString directory (void)
    { return baseUrl().path (KUrl::RemoveTrailingSlash); }
};
...
public:
  KPlayerEngine (KActionCollection* ac, QWidget* parent, KConfig*);
  ...
  static void initialize (KActionCollection* ac, QWidget* parent, KConfig* = 0);
